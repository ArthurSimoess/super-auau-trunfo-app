...
function CreateGameHeader () {
  ...
  return(
    <header className="p-3 w-screen flex flex-col justify-center gap-5 items-center bg-gradient-to-r from-stone-600 to-stone-900 text-white font-mono sm:flex-row" >
      ...
      <p>Nome do baralho: {<span className="text-yellow-400">{deckConfig.deckName}</span>}</p>
      <p>Criador do baralho: {<span className="text-yellow-400">{deckConfig.creatorName}</span>}</p>
    </header>
  )
}
